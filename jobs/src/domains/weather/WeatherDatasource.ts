...
export class WeatherDatasource {
    ...
    async getCurrentWeather(): Promise<WeatherCondition> {
        const response = await this.axiosClient.get('/stations/KFRG/observations/latest');

        return this.mapper.toWeatherCondition(response.data);
    }

    async getHourlyForecast(): Promise<Forecast> {
        const response = await this.axiosClient.get('/gridpoints/OKX/49,38/forecast/hourly');

        return this.mapper.toForecast(response.data);
    }

    async getDailyForecast(): Promise<Forecast> {
        const response = await this.axiosClient.get('/gridpoints/OKX/49,38/forecast');

        return this.mapper.toForecast(response.data);
    }
    async getAirQualityIndex(zipCode: string, distance: number): Promise<AirQualityData> {
        const response = await this.axiosClient.get(
            `/aq/observation/zipCode/current/?format=application/json&zipCode=${zipCode}&distance=${distance}&API_KEY=883E7AC5-FB4C-4368-8000-83428D2B6794`
        );
        return this.mapper.toAirQualityData(response.data);
    }
    async getOceanConditions(): Promise<OceanConditions> {
        const response = await this.axiosClient.get('/data/realtime2/44065.txt', {
            responseType: 'text'
        });
    ...
    async getTideData(startDate: Dayjs, endDate: Dayjs): Promise<TideData> {
        ...
        const response = await this.axiosClient.get(
            `/noaa/api/prod/datagetter?begin_date=${beginDate}&end_date=${endDateString}&station=${stationId}&product=predictions&time_zone=gmt&interval=hilo&units=metric&format=json&datum=MLLW`
        );
