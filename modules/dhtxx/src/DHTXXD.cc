...
static void _cb(int  event, int level, uint32_t tick, void *user)
{
...
DHTXXD_t *DHTXXD(int gpio, int model, DHTXXD_CB_t cb_func)
{
   DHTXXD_t *self;
   ...
   self->_last_edge_tick = gpioTick() - 10000;

   gpioSetAlertFuncEx(gpio, _cb, self);
...
void DHTXXD_auto_read(DHTXXD_t *self, float seconds)
{
   ...
   if (seconds > 0.0) self->_pth = gpioStartThread(pthTriggerThread, self);
}
